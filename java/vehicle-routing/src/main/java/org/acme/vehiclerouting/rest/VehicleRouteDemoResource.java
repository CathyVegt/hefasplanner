...
//            return loadDataWithParameters(LocalDate.now(), LocalDate.now().plusDays(1), LocalTime.of(8, 30), LocalTime.of(16, 30));
            return loadDataWithParameters(LocalDate.now(), LocalDate.now().plusDays(1), LocalTime.of(8, 30), LocalTime.of(16, 30),
                    "data/medewerkers.xlsx", "data/opdrachten.xlsx");
...
//            LocalTime shiftEnd) {
           LocalTime shiftEnd, String employeesFilePath, String visitsFilePath) {
...
//            DataParser.BrandTechPair brandTechPair =  DataParser.readEmployees();
            DataParser.BrandTechPair brandTechPair =  DataParser.readEmployees(employeesFilePath);
...
//            List<Visit> visits  = DataParser.readVisits(horizonStart, horizonEnd, brandCatalog);
            List<Visit> visits  = DataParser.readVisits(horizonStart, horizonEnd, brandCatalog, visitsFilePath);
            // create vehicles:
...
//                LocalDateTime maxEndTime = horizon[1].minusDays(n_days-i);
                ...
                LocalDateTime maxEndTime = horizonEnd.minusDays(n_days-i-1);
            ...
            for(Vehicle v: vehicles){
                System.out.println(v.getMinStartTime());
                System.out.println(v.getMaxEndTime());
            }
...
//        @QueryParam("shiftEnd") String shiftEnd) {
         @QueryParam("shiftEnd") String shiftEnd,
         @QueryParam("employeesFilePath") String employeesFilePath,
         @QueryParam("visitsFilePath") String visitsFilePath) {

        if (demoData == DemoData.LOADDATA && startDate != null && endDate != null && shiftStart != null && shiftEnd != null) {
            String resolvedEmployeesPath = (employeesFilePath == null || employeesFilePath.isBlank())
                    ? "data/medewerkers.xlsx"
                    : employeesFilePath;
            String resolvedVisitsPath = (visitsFilePath == null || visitsFilePath.isBlank())
                    ? "data/opdrachten.xlsx"
                    : visitsFilePath;

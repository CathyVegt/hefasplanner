...
public class VehicleRoutePlanResource {
    ...
    // Specific solving job
    private final ConcurrentMap<String, Job> jobIdToJob = new ConcurrentHashMap<>();

    // TODO Workaround to make Quarkus CDI happy. Do not use.
    public VehicleRoutePlanResource() {
        this.solverManager = null;
        this.solutionManager = null;
    }

    @Inject //TODO what is INJECT?
    ...
    @Operation(summary = "Export the route plan for a given job ID as an Excel file.")
    @APIResponses(value = {
            @APIResponse(responseCode = "200", description = "Excel export of the route plan."),
            @APIResponse(responseCode = "404", description = "No route plan found.",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON,
                            schema = @Schema(implementation = ErrorInfo.class))),
            @APIResponse(responseCode = "500", description = "Exception during solving a route plan.",

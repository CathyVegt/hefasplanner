...
const isoDatePattern = /^\d{4}-\d{2}-\d{2}$/; // ch
const timePattern = /^([01]\d|2[0-3]):[0-5]\d$/; //ch
...
//        const locations = vehicle.visits.map(visitId => visitByIdMap.get(visitId).location);
        const locations = vehicle.visits
            .map(function (visitReference) {
                const visitId = (visitReference && typeof visitReference === "object") ? visitReference.id : visitReference;
                return visitByIdMap.get(visitId);
            })
            .filter(function (visit) {
                return !!visit && !!visit.location;
            })
            .map(function (visit) {
...
//                                   </div>
//                                 </div>`
        const vehicleLabel = `<h5 class="card-title mb-1">Medewerker ${vehicle.id}</h5>`;
//        byVehicleGroupData.add({id: vehicle.id, content: vehicleWithLoad});
        byVehicleGroupData.add({id: vehicle.id, content: vehicleLabel});
    });
...
//    $.post("/route-plans", JSON.stringify(loadedRoutePlan), function (data) {
    if (!loadedRoutePlan) {
            alert("Please load demo data first before starting the solver.");
            return;
        }
...
function selectDemoData(item) { //start ch
    switchDataDropDownItemActive(item);
    scheduleId = null;
...
function loadRoutePlan(routePlan) {
    loadedRoutePlan = routePlan;
    refreshSolvingButtons(routePlan.solverStatus != null && routePlan.solverStatus !== "NOT_SOLVING");
    renderRoutes(routePlan);
    renderTimelines(routePlan);
...
//function loadDemoDataWithInputs(startDate, endDate, shiftStart, shiftEnd) {
//    const query = $.param({startDate, endDate, shiftStart, shiftEnd});
function loadDemoDataWithInputs(startDate, endDate, shiftStart, shiftEnd, employeesFilePath, visitsFilePath) {
    const query = $.param({startDate, endDate, shiftStart, shiftEnd, employeesFilePath, visitsFilePath});
    $.getJSON(`/demo-data/${demoDataId}?${query}`, function (routePlan) {
        loadRoutePlan(routePlan);
    }).fail(function (xhr, ajaxOptions, thrownError) {
        showError("Loading demo data failed.", xhr);
        refreshSolvingButtons(false);
    });
}

function isLoadDataInputValid(input) {
    return isoDatePattern.test(input.startDate)
        && isoDatePattern.test(input.endDate)
        && timePattern.test(input.shiftStart)
        && timePattern.test(input.shiftEnd)
        && !!input.employeesFilePath
        && !!input.visitsFilePath
        && input.startDate <= input.endDate;
}

function promptForLoadData() {
    if (!$.fn.modal) {
        promptForLoadDataFallback();
        return;
    }

    const loadDataModal = getOrCreateLoadDataModal();
    const form = loadDataModal.find("#loadDataForm");
    const submitButton = loadDataModal.find("#loadDataSubmitButton");
    const validationMessage = loadDataModal.find("#loadDataValidationMessage");

    const validateForm = function () {
        const input = {
            startDate: loadDataModal.find("#loadDataStartDate").val(),
            endDate: loadDataModal.find("#loadDataEndDate").val(),
            shiftStart: loadDataModal.find("#loadDataShiftStart").val(),
//            shiftEnd: loadDataModal.find("#loadDataShiftEnd").val()
            shiftEnd: loadDataModal.find("#loadDataShiftEnd").val(),
            employeesFilePath: loadDataModal.find("#loadDataEmployeesFilePath").val().trim(),
            visitsFilePath: loadDataModal.find("#loadDataVisitsFilePath").val().trim()

        };
        const valid = isLoadDataInputValid(input);
        submitButton.prop("disabled", !valid);
        validationMessage.toggleClass("d-none", valid);
        return {valid, input};
    };

    form.off("input").on("input", validateForm);
...
//        loadDemoDataWithInputs(input.startDate, input.endDate, input.shiftStart, input.shiftEnd);
        loadDemoDataWithInputs(input.startDate, input.endDate, input.shiftStart, input.shiftEnd, input.employeesFilePath, input.visitsFilePath);
    });
...
function promptForLoadDataFallback() {
    const startDate = window.prompt("Planning start date (YYYY-MM-DD):");
    const endDate = window.prompt("Planning end date (YYYY-MM-DD):");
    const shiftStart = window.prompt("Shift start time (HH:mm):");
    const shiftEnd = window.prompt("Shift end time (HH:mm):");
//    const input = {startDate, endDate, shiftStart, shiftEnd};
    const employeesFilePath = window.prompt("Technicians Excel path (for example data/medewerkers.xlsx):");
    const visitsFilePath = window.prompt("Visits Excel path (for example data/opdrachten.xlsx):");
    const input = {startDate, endDate, shiftStart, shiftEnd, employeesFilePath, visitsFilePath};

    if (!isLoadDataInputValid(input)) {
//        alert("All values are required. Use YYYY-MM-DD for dates, HH:mm for times, and keep startDate before or equal to endDate.");
        alert("All values are required. Use YYYY-MM-DD for dates, HH:mm for times, and keep startDate before or equal to endDate. Provide both Excel file paths.");
        return;
    }
//    loadDemoDataWithInputs(startDate, endDate, shiftStart, shiftEnd);
    loadDemoDataWithInputs(startDate, endDate, shiftStart, shiftEnd, employeesFilePath, visitsFilePath);
}

function getOrCreateLoadDataModal() {
    const existing = $("#loadDataModal");
    ...
    const defaultStartDate = toIsoDate(now);
    const defaultEndDate = toIsoDate(new Date(now.getTime() + 24 * 60 * 60 * 1000));

    const modal = $(
        `<div class="modal fade" id="loadDataModal" tabindex="-1" aria-labelledby="loadDataModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="loadDataModalLabel">Load planning data</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form id="loadDataForm">
                        <div class="modal-body">
                            <div class="mb-3">
                                <label class="form-label" for="loadDataStartDate">Start date</label>
                                <input class="form-control" id="loadDataStartDate" type="date" value="${defaultStartDate}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label" for="loadDataEndDate">End date</label>
                                <input class="form-control" id="loadDataEndDate" type="date" value="${defaultEndDate}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label" for="loadDataShiftStart">Shift start</label>
                                <input class="form-control" id="loadDataShiftStart" type="time" value="08:30" required>
                            </div>
                            <div class="mb-2">
                                <label class="form-label" for="loadDataShiftEnd">Shift end</label>
                                <input class="form-control" id="loadDataShiftEnd" type="time" value="16:30" required>
                            </div>
//                            <small id="loadDataValidationMessage" class="text-danger d-none">Provide all values in a valid format. Start date must be before or equal to end date.</small>
                            <div class="mb-3">
                                <label class="form-label" for="loadDataEmployeesFilePath">Technicians Excel file path</label>
                                <input class="form-control" id="loadDataEmployeesFilePath" type="text" value="data/medewerkers.xlsx" required>
                                <div class="form-text">Use a path relative to the app working directory or an absolute path.</div>
                            </div>
                            <div class="mb-2">
                                <label class="form-label" for="loadDataVisitsFilePath">Visits Excel file path</label>
                                <input class="form-control" id="loadDataVisitsFilePath" type="text" value="data/opdrachten.xlsx" required>
                            </div>
                            <small id="loadDataValidationMessage" class="text-danger d-none">Provide all values in a valid format. Start date must be before or equal to end date and both Excel file paths are required.</small>

                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                            <button id="loadDataSubmitButton" type="submit" class="btn btn-primary" disabled>Load data</button>
                        </div>
                    </form>
                </div>
            </div>
